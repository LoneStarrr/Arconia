...
/**
 * Unit tests for {@link PotMultiBlockPrimaryBlockEntity.TreeLocator}. Uses {@link
 * EphemeralTestServerProvider} for registry bootstrap and {@link MockLevel} for isolated block
 * storage per test.
 */
...
class TreeLocatorTests {
  ...
  private static void placeTree(
      MockLevel level, BlockPos basePos, RainbowColor color, int height, int width) {
    ...
    for (int y = 2; y < height; y++) {
      ...
      for (int dx = -width; dx <= width; dx++) {
        for (int dz = -width; dz <= width; dz++) {
          ...
          level.setBlock(
              layer.offset(dx, 0, dz),
              ModBlocks.getArconiumTreeLeaves(color).get().defaultBlockState(),
              0);
        }
      }
    }
  ...
  private static void placeGrass(MockLevel level, BlockPos pos, RainbowColor color) {
    for (int dx = -1; dx <= 1; dx++) {
      for (int dz = -1; dz <= 1; dz++) {
        ...
        level.setBlock(
            pos.offset(dx, 0, dz),
            ModBlocks.getRainbowGrassBlock(color).get().defaultBlockState(),
            0);
      }
    }
  }
  ...
  void testTreeLocatorSingleTree(MinecraftServer server) {
    ...
    assertEquals(
        121,
        tree.leaves().size(),
        "Should find 121 leaf blocks (7x7-1 at 2 trunk levels, 1 5x5 cap)");
  }
  ...
  void testTreeLocatorSingleTreeTooWide(MinecraftServer server) {
    ...
    assertEquals(
        145,
        tree.leaves().size(),
        "Should find 121 leaf blocks (7x7-1 at 2 trunk levels, 1 7x7 cap)");
  }
  ...
  void testTreeLocatorOffset(
      int dx,
      int dy,
      int dz,
      boolean expectedFound,
      boolean removeBottomTrunk,
