...
/**
 * Single 1.21.4 model provider replacing the old NeoForge BlockStateProvider + ItemModelProvider
 * pair.
 *
 * <p>Block state JSONs are generated for every mod block, each pointing at the matching
 * hand-authored model under {@code src/main/resources/assets/arconia/models/block/}. Item models
 * (the per-item layer textures) likewise reference the hand-authored item-model templates. The new
 * ClientItem JSONs (under {@code assets/arconia/items/}) are emitted for every item, including the
 * per-tier constant tint sources, the {@code minecraft:range_dispatch} for {@link
 * MagicInABottleFilledProperty}, and the {@code minecraft:special} composite for the colored
 * branches' {@link BranchItemRenderer}.
 */
public class ArconiaModelProvider extends ModelProvider {
  ...
  private void registerSharedItemModels(ItemModelGenerators itemModels) {
    ...
    ModelTemplates.FLAT_ITEM.create(
        prefix("item/arconium_essence"),
        TextureMapping.layer0(new Material(prefix("item/arconium_essence"))),
        itemModels.modelOutput);
    ModelTemplates.FLAT_ITEM.create(
        prefix("item/arconium_ingot"),
        TextureMapping.layer0(new Material(prefix("item/arconium_ingot_white"))),
        itemModels.modelOutput);
    ModelTemplates.TWO_LAYERED_ITEM.create(
        prefix("item/arconium_sickle"),
        TextureMapping.layered(
            new Material(prefix("item/sickle_handle")), new Material(prefix("item/sickle_head"))),
        itemModels.modelOutput);
    ModelTemplates.FLAT_ITEM.create(
        prefix("item/tree_branch_base"),
    ...
    ModelTemplates.TWO_LAYERED_ITEM.create(
        prefix("item/arconium_tree_sapling"),
        TextureMapping.layered(
            new Material(prefix("block/sapling_white")),
            new Material(prefix("block/sapling_speckles"))),
        itemModels.modelOutput);
  }
    ...
    TextureMapping mapping =
        new TextureMapping()
            .put(
                ...
                new Material(Identifier.withDefaultNamespace("block/oak_planks")))
            .put(
                net.minecraft.client.data.models.model.TextureSlot.TOP,
                new Material(prefix("block/world_builder")));
  ...
  private void registerSingletonBlocks(BlockModelGenerators blockModels) {
    ...
    simpleBlockState(
        blockModels, ModBlocks.potMultiBlockPrimary.get(), prefix("block/pot_multiblock_primary"));
    ...
    simpleBlockState(
        blockModels, ModBlocks.potMultiBlockSecondary.get(), prefix("block/empty_model"));
  }
  ...
  private void registerTieredBlocks(BlockModelGenerators blockModels) {
    for (RainbowColor color : RainbowColor.values()) {
      ...
      simpleBlockState(
          blockModels,
          ModBlocks.getArconiumTreeLeaves(color).get(),
          prefix("block/arconium_tree_leaves"));
      simpleBlockState(
          blockModels,
          ModBlocks.getArconiumTreeSapling(color).get(),
          prefix("block/cross_sapling"));
      simpleBlockState(
          blockModels, ModBlocks.getArconiumBlock(color).get(), prefix("block/arconium_block"));
      simpleBlockState(
          blockModels,
          ModBlocks.getRainbowGrassBlock(color).get(),
          prefix("block/rainbow_grass_block"));
      ...
      tintedFlatItem(
          ...
          ModBlocks.getArconiumTreeLeaves(color).get().asItem(),
          assets.itemModelLeaves(),
          assets.tint());
      tintedFlatItem(
          blockModels.itemModelOutput,
          ModBlocks.getArconiumTreeSapling(color).get().asItem(),
          assets.itemModelSapling(),
          assets.tint(),
          saplingOverlayTint());
      tintedFlatItem(
          blockModels.itemModelOutput,
          ModBlocks.getArconiumBlock(color).get().asItem(),
          assets.itemModelArconiumBlock(),
          assets.tint());
      tintedFlatItem(
          blockModels.itemModelOutput,
          ModBlocks.getRainbowGrassBlock(color).get().asItem(),
          assets.itemModelGrassBlock(),
          assets.tint());
    }
  }
  ...
  private void registerStaticItems(ItemModelGenerators itemModels) {
    ...
    itemModels.itemModelOutput.accept(
        ModItems.cloverStaff.get(), ItemModelUtils.plainModel(prefix("item/clover_staff")));
    itemModels.itemModelOutput.accept(
        ModItems.fourLeafClover.get(), ItemModelUtils.plainModel(prefix("item/four_leaf_clover")));
    itemModels.itemModelOutput.accept(
        ModItems.threeLeafClover.get(),
        ItemModelUtils.plainModel(prefix("item/three_leaf_clover")));
  }
  ...
  private void registerTieredItems(ItemModelGenerators itemModels) {
    ...
    for (RainbowColor color : RainbowColor.values()) {
      ...
      tintedFlatItem(
          ...
          ModItems.getArconiumEssence(color).get(),
          prefix("item/arconium_essence"),
          assets.tint());
      tintedFlatItem(
          itemModels.itemModelOutput,
          ModItems.getArconiumIngot(color).get(),
          prefix("item/arconium_ingot"),
          assets.tint());
      tintedFlatItem(
          itemModels.itemModelOutput,
          ModItems.getArconiumSickle(color).get(),
          prefix("item/arconium_sickle"),
          sickleHandleTint(),
          assets.tint());
    }
  ...
  private void registerMagicInABottle(ItemModelGenerators itemModels) {
    ...
    ItemModel.Unbaked fallback =
        ItemModelUtils.tintedModel(prefix("item/magic_in_a_bottle"), layer0Untinted, swirlTint);
    List<RangeSelectItemModel.Entry> entries =
        List.of(
            ItemModelUtils.override(
                ItemModelUtils.tintedModel(
                    prefix("item/magic_in_a_bottle_filled1"), layer0Untinted, swirlTint),
                0.12f),
            ItemModelUtils.override(
                ItemModelUtils.tintedModel(
                    prefix("item/magic_in_a_bottle_filled2"), layer0Untinted, swirlTint),
                0.25f),
            ItemModelUtils.override(
                ItemModelUtils.tintedModel(
                    prefix("item/magic_in_a_bottle_filled3"), layer0Untinted, swirlTint),
                0.37f),
            ItemModelUtils.override(
                ItemModelUtils.tintedModel(
                    prefix("item/magic_in_a_bottle_filled4"), layer0Untinted, swirlTint),
                0.50f),
            ItemModelUtils.override(
                ItemModelUtils.tintedModel(
                    prefix("item/magic_in_a_bottle_filled5"), layer0Untinted, swirlTint),
                0.62f),
            ItemModelUtils.override(
                ItemModelUtils.tintedModel(
                    prefix("item/magic_in_a_bottle_filled6"), layer0Untinted, swirlTint),
                0.75f),
            ItemModelUtils.override(
                ItemModelUtils.tintedModel(
                    prefix("item/magic_in_a_bottle_filled7"), layer0Untinted, swirlTint),
                0.87f),
            ItemModelUtils.override(
                ItemModelUtils.tintedModel(
                    prefix("item/magic_in_a_bottle_filled8"), layer0Untinted, swirlTint),
                1.00f));
    ItemModel.Unbaked rangeSelect =
        ItemModelUtils.rangeSelect(new MagicInABottleFilledProperty(), fallback, entries);
  ...
  private void registerColoredBranches(ItemModelGenerators itemModels) {
    ...
    for (RainbowColor color : RainbowColor.values()) {
      ...
      ItemModel.Unbaked overlay =
          ItemModelUtils.specialModel(base, new BranchItemRenderer.Unbaked());
  ...
  private void simpleBlockState(
      BlockModelGenerators blockModels, Block block, Identifier modelLocation) {
    blockModels.blockStateOutput.accept(
        BlockModelGenerators.createSimpleBlock(
            block, BlockModelGenerators.plainVariant(modelLocation)));
  }

  /**
   * Constant per-layer tint for a flat (sprite-based) item model living at {@code modelLocation}.
   */
  private void tintedFlatItem(
      ItemModelOutput output, Item item, Identifier modelLocation, ItemTintSource... tints) {
