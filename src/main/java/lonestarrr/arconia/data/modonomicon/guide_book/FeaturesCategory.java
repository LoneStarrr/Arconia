...
public class FeaturesCategory extends CategoryProvider {
  ...
  protected void generateEntries() {
    ...
    // the condition for the level 1 entry to depend on the root entry is set up here so we can
    // access the entry. We could also do it in the entry provider and either hand over a reference,
    // or use the ID as resource location to reference it
    var conditionLevel1Entry =
        this.add(new ConditionLevel1Entry(this).generate('1'))
    ...
    var conditionLevel2Entry =
        this.add(new ConditionLevel2Entry(this).generate('2'))
    ...
    var twoParentsEntry =
        this.add(new TwoParentEntry(this).generate('t'))
    ...
    var spotlightEntry =
        this.add(new SpotlightEntry(this).generate('s'))
            .withParent(this.parent(recipeEntry).withLineReversed(true));

    var componentIconEntry =
        this.add(new EntryWithComponentIcon(this).generate('x')).withParent(spotlightEntry);

    var emptyEntry = this.add(new EmptyPageEntry(this).generate('e')).withParent(spotlightEntry);
    ...
    var customIconEntry = this.add(new CustomIconEntry(this).generate('g')).withParent(imageEntry);
  }
    ...
    return category
        .withBackgroundParallaxLayers(
            new BookCategoryBackgroundParallaxLayer(
                this.modLoc("textures/gui/parallax/flow/base.png"), 0.7f, -1),
            new BookCategoryBackgroundParallaxLayer(
                this.modLoc("textures/gui/parallax/flow/1.png"), 1f, -1),
            new BookCategoryBackgroundParallaxLayer(
                this.modLoc("textures/gui/parallax/flow/2.png"), 1.4f, -1))
        .withCategoryButtonSprites(
            Identifier.fromNamespaceAndPath(
                "modonomicon",
                "modonomicon/themes/default/content/buttons/category/category_button_golden"),
            Identifier.fromNamespaceAndPath(
                "modonomicon",
                "modonomicon/themes/default/content/buttons/category/category_button_golden"),
            44,
