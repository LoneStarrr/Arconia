 ...
 * after touching anything that affects generated data) to refresh files under
 * {@code src/generated/resources}, which are picked up automatically when the {@code runClient}
 * task launches.
 *
 * <p>1.21.4 NeoForge splits {@code GatherDataEvent} into {@code Server} and {@code Client}
 * subclasses. Per the 21.4 release-notes recommendation, we generate everything in a single
 * {@code clientData} run (configured in {@code build.gradle}) and listen only on the {@code Client}
