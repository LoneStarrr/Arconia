...
public class WorldBuilderEntity extends BaseBlockEntity {
  ...
  private static final InputStream stream =
      WorldBuilderEntity.class.getResourceAsStream(
          "/data/arconia/world_builder/world_builder.json");
  ...
  private int blockLevels =
      7; // This many vertical levels to convert, starting with the level directly below the builder
  private int gameTicksPerBuilderTick = 1; // how often to tick
  private float boostFactor =
      1F; // Multiplication factor to increase chance to convert into boostable blocks (typically
  // non-stone blocks)

  private BlockState blockToMatch =
      Blocks.OAK_PLANKS.defaultBlockState(); // The block the builder is supposed to convert
  private ComputedDistribution
      currentDistribution; // Calculated weight-based distribution for current y level in process of
  // being converted
  private List<BlockPos> blocksToConvert =
      null; // Calculated list of block positions in process of being converted
  ...
  public static void tick(
      Level level, BlockPos pos, BlockState state, WorldBuilderEntity blockEntity) {
  ...
  private void tickInternal() {
    ...
    for (int i = 0; i < blocksPerTick; i++) {
      ...
      if (currentDistribution == null || convertPos.getY() != currentDistribution.yLevel) {
        currentDistribution =
            new ComputedDistribution(
                distributionTable.distributions.get("overworld"),
                convertPos.getY(),
                this.boostFactor,
                level.getRandom());
        if (Arconia.logger.isDebugEnabled()) {
          Arconia.logger.debug(
              "Calculating new distribution for y="
                  + convertPos.getY()
                  + ": "
                  + currentDistribution);
        }
      ...
      level.playSound(
          null, convertPos, converted.getSoundType().getPlaceSound(), SoundSource.BLOCKS, 1, 1);
      currentBlockIndex++;
    }
  }

  /**
   * Recursive function to find all blocks of a specific blockstate that are connected through one
   * of the cardinal directions and up/down
   *
   * @param pos
   * @param toMatch
   * @param neighbors
   ...
   */
  private void findConnectedNeighbors(
      BlockPos pos,
      final BlockState toMatch,
      final Set<BlockPos> neighbors,
  ...
  private Set<BlockPos> findConnectedBlocks(BlockState state) {
    ...
    // Define the bounds of the area, which is defined by 2 blocks since it's a cuboid shape. The
    // cuboid sits directly
    // underneath the world builder, with the builder sitting in the middle of the horizontal face.
    BlockPos minPos =
        startPos.offset(
            -this.blockRadiusHorizontal, -(this.blockLevels - 1), -this.blockRadiusHorizontal);
    ...
    Set<BlockPos> connectedBlocks =
        new HashSet<>((this.blockRadiusHorizontal * 2 + 1) * this.blockLevels);
...
/**
 * Represents a list of blocks to randomly select from for a specific Y level - the builder will
 * replace blocks per y level, and caching this per y level should make it not horribly inefficient.
 */
    ...
    for (DistributionEntry entry : entries) {
      if (entry.yMin <= yLevel && entry.yMax >= yLevel) {
        ...
        if (entry.distribution == DistributionType.CENTER) {
          // Based on that famous 1.18 minecraft ore distribution chart: Weight is highest in the
          // middle, then linearly decreased the further out you go
...
class DistributionEntry {
  ...
  @SerializedName("deepslate")
  @JsonAdapter(BlockJsonAdapter.class)
  public Block deepslateBlock;
...
class DistributionTable {
  ...
  @Override
  public String toString() {
    return "DistributionTable{" + "distributions=" + distributions + '}';
  }
}
