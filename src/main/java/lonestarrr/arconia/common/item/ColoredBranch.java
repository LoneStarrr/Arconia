...
public class ColoredBranch extends Item {
  ...
  /**
   * Colored branches can be imbued through a ritual with a specific item. Once imbued,
   * right-clicking the branch near an activated resource tree will have that tree produce this
   * resource.
   *
  ...
  public static void setResourceItem(
    ...
    coloredBranchStack.set(
        DataComponents.CONTAINER,
        ItemContainerContents.fromItems(new ArrayList<>(List.of(resourceItem))));
  }
  ...
  /**
   * Create a base ItemStackTemplate for a colored branch. Safe to call during datagen when
   * registries aren't fully loaded. The resource item is set separately via PedestalRecipe's
   * resourceItem field.
   */
  public static ItemStackTemplate getColoredBranchTemplate(RainbowColor color) {
    Item branch = ModItems.getColoredBranch(color).get();
    return new ItemStackTemplate(branch);
  }
  ...
  public @NotNull InteractionResult useOn(UseOnContext context) {
    ...
    if (branch.has(DataComponents.CONTAINER)) {
      // Imbued branches can't be used to color grass as it would consume the branch and that would
      // make some
      // users a bit angry.
      return InteractionResult.PASS;
    }
    // Attempt to change the block being touched to a rainbow grass block of the same color as the
    // branch
