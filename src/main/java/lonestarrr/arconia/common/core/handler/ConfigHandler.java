...
public final class ConfigHandler {
  ...
  public static class Common {
    public final Map<RainbowColor, ModConfigSpec.IntValue> potGenerationInterval =
        new HashMap<>(RainbowColor.values().length);
    public final Map<RainbowColor, ModConfigSpec.IntValue> potGenerationCount =
        new HashMap<>(RainbowColor.values().length);
    public final Map<RainbowColor, ModConfigSpec.IntValue> leavesItemCredits =
        new HashMap<>(RainbowColor.values().length);
    ...
    public Common(ModConfigSpec.Builder builder) {
      ...
      // Each next tier doubles throughput. Keep credit increase below doubling so that player needs
      // to replant
      // trees faster with the later tiers. This will make earlier tiers feasible with manual tree
      // planting,
      // but higher tiers require automation to keep going indefinitely.
      // Smaller trees have ~50 leaves, large ones ~100. So if a pot consumes a small purple tree,
      // it produces
      // 8 items per 2 seconds (4/s), with 12 credits per leaf makes it runs out in 50*12/4=150
      // seconds. Meaning
      // one has to replant it every 2.5 minutes worst case.
      // A small red tree producing 1 item every 16 seconds with 1 credit per leaf lasts
      // 50*1/(1/16.) 800 seconds.
      final int[] itemCreditsFromLeaves = {
        1, 2, 3, 5, 7, 9, 12
      }; // #items produced per consumed leaves block
      ...
      for (RainbowColor color : RainbowColor.values()) {
        ...
        ModConfigSpec.IntValue generationInterval =
            builder
                .comment("Time between item generation attempts, in game ticks")
                .defineInRange(
                    colorLabel + "GenerationInterval", currentGenerationInterval, 5, 1200);
        potGenerationInterval.put(color, generationInterval);

        ModConfigSpec.IntValue generationCount =
            builder
        ...
        ModConfigSpec.IntValue itemCredits =
            builder
                .comment("Number of items extracted from the pot for each leaves block consumed")
                .defineInRange(
                    colorLabel + "ItemCreditsFromLeaves", itemCreditsFromLeaves[tier - 1], 1, 256);
      ...
      bonusPerExtraTree =
          builder
              .comment(
                  "Extra item draw bonus chance percentage for each unique tree color of a previous tier planted nearby")
