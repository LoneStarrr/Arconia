 ...
 *
 * <p>Example run with 2 rules and 3 iterations: /fractal-tree "0:-1>W0<>N0<>E0<>S0<0+,1:11" 3
 *
 * <p>http://algorithmicbotany.org/papers/abop/abop.pdf - page 27 and onwards are interesting,
 * describing how to introduce randomness
 */
public class FractalTreeCommand {
  public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
    dispatcher.register(
        Commands.literal("fractal-tree")
            .then(
                Commands.argument("rules", StringArgumentType.string())
                    .then(
  ...
  private static int fractalTree(
      CommandContext<CommandSourceStack> ctx, String rulesSerialized, int iterations)
  ...
  private static Map<Character, String> parseRules(String serialized)
      throws CommandSyntaxException {
  ...
  private static void renderLTree(
      Level worldIn, BlockPos posIn, String ltree, Player player, int stopAt) {
    int count = 0;
      ...
      switch (c) {
        ...
        case '0': // place leaf
          ...
          worldIn.setBlockAndUpdate(
              s.pos, Blocks.OAK_LEAVES.defaultBlockState().setValue(LeavesBlock.DISTANCE, 1));
          break;
        case 'W': // rotate "west"
        ...
        case '+': // grow logs drawn
          try {
            ...
            Arconia.logger.error(
                "Empty logsize stack while drawing ltree: "
                    + ltree.substring(0, count + 1)
                    + " of "
                    + ltree);
          }
          break;
