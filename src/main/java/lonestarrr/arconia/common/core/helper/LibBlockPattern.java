...
public abstract class LibBlockPattern {
  public static List<List<BlockState>> readBlockPattern(Identifier location)
  ...
  public static List<List<BlockState>> readBlockPattern(String jsonString)
  ...
  private static List<List<BlockState>> parseJson(JsonObject json) throws BlockPatternException {
    ...
    // TODO: why not make a class representing the entire data structure including
    // pattern/blocks/..?
    ...
    try {
      Type patternType = new TypeToken<List<String>>() {}.getType();
    ...
    try {
      Type blocksType = new TypeToken<Map<String, BlockDefinition>>() {}.getType();
    ...
    for (Map.Entry<String, BlockDefinition> blockDefEntry : blockDefinitionMap.entrySet()) {
      ...
      blockMap.put(
          blockDefEntry.getKey(), LibBlockPattern.deserializeBlockState(blockDef.getBlockId()));
    }
  ...
  /**
   * Given a block by name (e.g. minecraft:wool) and a map of metadata (e.g. {"color":"lime"},
   * produce a BlockState.
   *
