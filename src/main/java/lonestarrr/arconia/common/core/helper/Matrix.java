...
public abstract class Matrix {
   ...
   * @param rotatedMatrix Rotated matrix elements are stored in this matrix. It must have the
   *     correct dimensions (NxM)
   */
  public static <T> void rotate2DMatrixClockWise(final T[][] matrix, final T[][] rotatedMatrix) {
    int rows = matrix.length;
    int columns = matrix[0].length;

    // Passed in rotatedMatrix is necessary because with a Generics type one cannot create a new
    // array without
    // knowing the type
  ...
  /**
   * Given a coordinate in a 3d matrix, calculate the new coordinate in that matrix after a given
   * amount of clockwise 90 degree rotations of a horizontal plane along the x/z axis
   *
   ...
   */
  public static Vec3i getRotatedCoordinate(
      final Vec3i coordinate,
