...
public class OutlineBlockRenderer {
    ...
    static {
        ...
        RenderState.LayerState projectionLayering = ObfuscationReflectionHelper.getPrivateValue(RenderState.class, null, "field_239235_M_");
        ...
        RenderType.State glState = RenderType.State.getBuilder().line(new RenderState.LineState(OptionalDouble.of(3))).layer(projectionLayering).transparency(TRANSLUCENT_TRANSPARENCY).writeMask(colorMask).depthTest(noDepth).build(false);
        LINE_3_NO_DEPTH = RenderType.makeType(Arconia.MOD_ID + ":line_3_no_depth", DefaultVertexFormats.POSITION_COLOR, GL11.GL_LINES, 128, glState);
        ...
        LINE_8_NO_DEPTH = RenderType.makeType(Arconia.MOD_ID + ":line_8_no_depth", DefaultVertexFormats.POSITION_COLOR, GL11.GL_LINES, 128, glState);
        // https://github.com/Vazkii/Botania/blob/master/src/main/java/vazkii/botania/client/core/handler/BoundTileRenderer.java
        LINE_BUFFERS = IRenderTypeBuffer.getImpl(Util.make(() -> {
            Map<RenderType, BufferBuilder> ret = new IdentityHashMap<>();
            ret.put(LINE_3_NO_DEPTH, new BufferBuilder(LINE_3_NO_DEPTH.getBufferSize()));
    ...
    public static void renderOutline(@Nonnull final MatrixStack matrixStack, @Nonnull final BlockPos pos, float hue, RenderType lineType) {
        ...
        List<AxisAlignedBB> list = shape.toBoundingBoxList().stream().map(bl -> bl.offset(pos)).collect(Collectors.toList());
        if (!list.isEmpty()) {
            matrixStack.scale(1F, 1F, 1F);
            for (AxisAlignedBB axis : list) {
                axis = axis.offset(-pos.getX(), -pos.getY(), -(pos.getZ() + 1));
                renderBlockOutline(matrixStack.getLast().getMatrix(), buffer, axis, color);
            }
        }
        matrixStack.pop();
