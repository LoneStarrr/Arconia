...
public class RootItemRenderer extends BlockEntityWithoutLevelRenderer {
    public static final ModelResourceLocation TREE_ROOT_BASE_MODEL = ModelResourceLocation.standalone(ResourceLocation.fromNamespaceAndPath(Arconia.MOD_ID, "item/tree_root_base"));
    ...
    public void renderByItem(@NotNull ItemStack stack, @NotNull ItemDisplayContext displayContext, PoseStack poseStack, @NotNull MultiBufferSource buffer, int packedLight, int packedOverlay) {
        /* If this root is imbued with an item, render a small version of the item behind the root to differentiate it
         * from a plain root, and also to quickly see what item it will have the pot of gold generate.
         */
        ...
        // The root texture is an item model, but it's not associated with an item, thus custom loaded.
        BakedModel bakedModel = minecraft.getModelManager().getModel(TREE_ROOT_BASE_MODEL);
