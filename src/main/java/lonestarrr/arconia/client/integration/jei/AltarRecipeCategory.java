...
public class AltarRecipeCategory implements IRecipeCategory<PedestalRecipe> {
  ...
  public AltarRecipeCategory(@Nonnull IGuiHelper guiHelper) {
    this.overlay =
        guiHelper.createDrawable(
            Identifier.fromNamespaceAndPath(Arconia.MOD_ID, "textures/gui/jei/altar_overlay.png"),
            0,
            0,
            144,
            81);
  ...
  public void setRecipe(IRecipeLayoutBuilder builder, PedestalRecipe recipe, IFocusGroup focuses) {
    ...
    for (int i = 0; i < inputs.size(); i++) {
      inputSlots
          .get(i)
          .addItemStacks(inputs.get(i).items().map(holder -> new ItemStack(holder)).toList());
    }
    builder.addSlot(RecipeIngredientRole.OUTPUT, 121, 33).add(recipe.getOutput());
  }
}
