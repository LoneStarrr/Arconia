...
public class ItemProjector {
   ...
   */
  public static void projectItem(
      ItemStack stack,
      BlockPos itemPos,
      PoseStack poseStack,
    ...
    if (!forceShow) {
      ...
      if (!shape.isEmpty()
          && shape
              .bounds()
              .move(itemPos)
              .contains(itemPos.getX(), itemPos.getY(), itemPos.getZ())) {
        return;
      }
    }
  ...
  /**
   * Displays a carousel of multiple items, where the carousel itself rotates around its center
   ...
   * @param itemsPerLevel How many items to display per y layer. If there are more items, a new y
   *     layer will be rendered above
   * @param levelOffset Additional render offset (x+y) for each extra y layer
   */
  public static void projectItemCarousel(
    ...
    float rotationCarousel =
        (level.getGameTime() % ticksPerRotation) / (float) ticksPerRotation * 360f;
    poseStack.pushPose();
  ...
  private static void submitItemStack(
      ItemStack stack,
      PoseStack poseStack,
      SubmitNodeCollector nodeCollector,
      int lightCoords,
    ...
    minecraft
        ...
        .updateForTopItem(state, stack, ItemDisplayContext.GROUND, minecraft.level, null, 0);
