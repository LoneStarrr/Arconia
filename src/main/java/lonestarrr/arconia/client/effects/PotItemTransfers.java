...
public class PotItemTransfers {
  ...
  public static void render(SubmitCustomGeometryEvent event) {
    ...
    for (ItemTransfer transfer : transfers) {
      ...
      if (playerPos.distToCenterSqr(transfer.startPos.x, transfer.startPos.y, transfer.startPos.z)
          > 64 * 64) {
        continue;
      }
  ...
  private static void renderItemTransfer(
    ...
    //        int light = level.getLightEmission(new BlockPos((int)itemPos.x, (int)itemPos.y,
    // (int)itemPos.z));
    ...
    Minecraft.getInstance()
        ...
        .updateForTopItem(state, toRender, ItemDisplayContext.GROUND, level, null, 0);
...
class ItemTransfer {
  ...
  public ItemTransfer(Vec3 startPos, Vec3 endPos, ItemStack itemStack, long startTick) {
    ...
    this.displayTicks =
        DISPLAY_TICKS * (0.6 + 0.4 * Minecraft.getInstance().level.getRandom().nextFloat());
  }
  ...
  public Vec3 getPosition(double elapsedTicks) {
    ...
    double itemY =
        endPos.y + velocity.y * elapsedTicks - (gravity * elapsedTicks * elapsedTicks) / 2d;
    double itemZ = endPos.z + velocity.z * elapsedTicks;
    return new Vec3(itemX, itemY, itemZ);
  }

  /**
   * @return A velocity vector V for a parabolic animation of an item being flung at the hat where
   *     the fling time is constant.
   */
  public Vec3 getVelocity() {
    final double vx = (startPos.x - endPos.x) / displayTicks;
    final double vz = (startPos.z - endPos.z) / displayTicks;
    final double vy =
        (startPos.y - endPos.y + (gravity * displayTicks * displayTicks) / 2d) / displayTicks;
  ...
  public float getTimeElapsedPct() {
    return Math.min(
        1f, (Minecraft.getInstance().level.getGameTime() - startTick) / (float) DISPLAY_TICKS);
  }
  ...
  /**
   * @return Distance traveled % as a fraction [0..1] - not the same as time elapsed as the speed is
   *     non-linear.
   */
